...
/*   Updated: 2024/11/02 06:04:44 by mogawa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Epoller::loop()
{
	while (true)
	{
		int	num_event = epollWait();
		for (int i = 0; i < num_event; i++)
		{
			uint32_t	ev = event_list_[i].events;
			ASocket		*socket = static_cast<ASocket*>(event_list_[i].data.ptr);
			ft::State	state;
			socket->execute();
			SocketHolder::deleteMarkedSocket();
		}
